...
class Agent():
    def __init__(self, computation_graph_args, sample_trajectory_args, estimate_return_args):
        ...
        if self.discrete == False:
            self.sy_logstd = tf.Variable(initial_value=tf.zeros(shape=self.ac_dim, dtype = tf.dtypes.float32))
    ...
    def sample_action(self, policy_parameters):
        ...
        """
        if self.discrete:
            sy_sampled_ac = tf.random.categorical(policy_parameters, 1)
            sy_sampled_ac = tf.reshape(sy_sampled_ac, [-1]).numpy()
        else:
            sy_mean, sy_logstd = policy_parameters
            sy_std = tf.math.exp(sy_logstd)
            actions = tf.random.normal(sy_mean.shape)
            actions = sy_mean + actions * sy_std
            sy_sampled_ac = actions.numpy()
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        """
        if self.discrete:
            sy_logits_na = policy_parameters  # (batch_size, self.ac_dim)
            sy_ac_na = tf.dtypes.cast(sy_ac_na, tf.int32)
            # raw way for log probability -tf.reduce_sum(tf.nn.log_softmax(logits), tf.one_hot(labels))
            #logits = tf.nn.log_softmax(sy_logits_na)
            #labels = tf.one_hot(sy_ac_na, depth=self.ac_dim)
            #sy_logprob_n1 = - tf.reduce_sum(tf.multiply(logits, labels), axis = 1)
            # tensorflow
            sy_logprob_n2 = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = sy_logits_na, labels = sy_ac_na)
            sy_logprob_n = sy_logprob_n2
        else:
            sy_mean, sy_logstd = policy_parameters
            sy_z = tf.square((sy_mean - sy_ac_na) / (tf.math.exp(sy_logstd) + 1e-8))
            sy_z = -0.5*(sy_z + 2*sy_logstd + np.log(2*np.pi))
            sy_logprob_n = tf.reduce_sum(sy_z, axis = 1)
        return sy_logprob_n
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            #                           ----------PROBLEM 3----------
            #====================================================================================#
            ac = self.sample_action(self.policy_forward_pass(ob[None,:].astype(np.float32)))
            ac = ac[0]
            acs.append(ac)
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.discrete:
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                loss2 = -tf.reduce_mean(tf.multiply(loss1, adv_n))
            gradients = tape.gradient(loss2, self.model.trainable_variables)
            gradients2 = tape.gradient(loss2, self.sy_logstd)
            self.update_op.apply_gradients(zip(gradients, self.model.trainable_variables))
            self.update_op.apply_gradients(zip([gradients2], [self.sy_logstd]))
            del tape
        return loss2.numpy()
...
def train_PG(
    ...
    for itr in range(n_iter):
        ...
        after_loss = agent.update_parameters(ob_no, ac_na, q_n, adv_n)
